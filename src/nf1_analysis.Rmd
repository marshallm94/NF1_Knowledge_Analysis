...
Although the variance of test scores are very close to each other, the sample sizes are not (see *Table 1. Study Aim 1: Sample Statistics*), leading to the choice of Welch's *t*-test as the statistical test to be used.

These three tests, combined with the test for the second study aim, which uses Welch's *t*-test for the same reasons as mentioned above (see *Table 2. Study Aim 2: Sample Statistics*), created a total of 4 statistical tests. In order to control the Family-Wise Type I error rate, the original significance level ($\alpha$ = `r alpha`) was adjusted using the Bonferroni Correction, reducing the level of statistical significance to $\alpha_{adj}$ = `r adj.alpha`.
...
ggplot(test.1.data, aes(score, fill = group_id)) +
    ...
    geom_vline(xintercept = mean(unaffected$score), color = 'darkgreen') +
    geom_vline(xintercept = mean(other.parent.affected$score), color = 'orange') +
...
subtitle <- paste("Mean's of each group displayed as vertical lines: Group A =", round(mean(unaffected$score), 4), "|", "Group C =", round(mean(affected$score), 4))
...
subtitle <- paste("Mean's of each group displayed as vertical lines: Group B =", round(mean(other.parent.affected$score), 4), "|", "Group C =", round(mean(affected$score), 4))
...
```{r}
tiff(filename = '../images/GeneticCounselingExposure.png',
     units = 'in',
     width = 8,
     height = 5,
     res = 300)
...
title(main = 'Comparison of NF1 Knowledge and Exposure to Genetic Counseling',
      ylab = 'Survey Score',
...
Since there are three groups that need to be tested (Affected, Other Parent Affected and Unaffected), starting with an ANOVA test will determine whether there is any evidence that at least one group mean is different from the others.

Shown below, the ANOVA results show a highly significant result with a p value = 0.000511. Following this up with Tukey's Honestly Significant Difference (HSD) test, it is shown that two of the three pairwise comparisons have statistically significant different means - namely, that Affected group has a lower average score than both the other groups.
...
tukey.hsd.result$group_id
```

```{r anova}
tiff(filename = '../images/BoxWhiskers.png',
     units = 'in',
     width = 8,
     height = 5,
     res = 300)
boxplot(score ~ group_id,
        data = grand.data)
title(main = 'One Way ANOVA: p value < 0.001',
      ylab = 'Survey Score',
      xlab = 'Group',
      font.main = 4,
      font.lab = 2)
...
tiff(filename = '../images/PairwiseComparison.png',
     units = 'in',
     width = 8,
     height = 5,
     res = 300)
...
legend('topright', legend=c('Affected','Other Parent Affected','Unaffected'), fill=legend_col)
axis(side=2, at=pretty(seq(0.5, 1, 0.1)), labels=paste0(pretty(seq(0.5, 1, 0.1)*100), "%"), las=2)
...
```{r}
unaffected.normality <- shapiro.test(unaffected$score)$p.value
other.parent.affected.normality <- shapiro.test(other.parent.affected$score)$p.value
affected.normality <- shapiro.test(affected$score)$p.value
seen.gc.normality <- shapiro.test(seen.gc$score)$p.value
not.seen.gc.normality <- shapiro.test(not.seen.gc$score)$p.value

summary.df <- data.frame(PValue = c(unaffected.normality,
                                    other.parent.affected.normality,
                                    affected.normality,
                                    seen.gc.normality,
...
A total of 5 statistical tests were performed, broken into two “families” of tests, one family for the first study aim and one family for the second study aim. The first family of tests consisted of a one way ANOVA test followed by 3 post hoc pairwise comparisons. The second “family” consisted of one test - Welch’s one-sided t-test. An important note is that the number of tests in a family can increase the Family-Wise Type I error rate. Tukey’s HSD, the method used in this study to control the Family-Wise Type I error rate,  is one of many possible solutions to the problem of multiple comparisons and, "there is no universally accepted approach for dealing with the problem of multiple comparisons" (McDonald, 2014)
