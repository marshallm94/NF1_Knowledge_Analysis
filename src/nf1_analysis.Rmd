...
It is important to note that the answers to the below question, "What specialists does your affected child see?" are not mutually exclusive - children who sa a Neurologist may have seen an Oncologist and a Geneticist as well. 
...
display.df <- data.frame(table(grand.data$mode_of_inheritance, grand.data$how_often_does_your_.oldest._child_see_an_nf_doctor.))

colnames(display.df) <- c('ModeOfInheritance','Frequency','Count')

ggplot(display.df, aes(x = Frequency,
                       y = Count,
                       fill = Frequency)) +
    geom_bar(stat = 'identity') +
    facet_grid(~ModeOfInheritance) +
    geom_text(aes(y = Count/tappy(..PANEL..,sum),
                  label = Count),
              stat = 'identity',
              vjust = -0.25)
    geom_text(aes(y = ((..count..)/sum(..count..)),
